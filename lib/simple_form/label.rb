module SimpleForm
  module Label
    ...
    def label_text
      required_text << (@option[:label] || translate_label)
    end

    def required_text
      attribute_required? ? translate_requred_string : ''
    end
    ...
    def translate_required_string
      translate_string(:required_string, :default => %[<abbr title="#{translate_required_text}">#{translate_required_mark}</abbr>])
    end
